...
public class ParticipateProjectController {
     ...
     * 获取学生参加项目的详细信息
     */
    @GetMapping("/getAcademicPerformance/getParticipateProjectDetail")
    public Result getParticipateProjectDetail(@RequestParam String studentNumber,@RequestParam String projectName){
        log.info("管理端根据学生学号和项目名称查询学生参加项目的详细信息");
        return Result.success(participateProjectService.getOne(new QueryWrapper<ParticipateProject>().eq("student_number",studentNumber).eq("project_name",projectName)));
    }
    /**
     * 新增学生参加项目
     */
     ...
     * 导出学生参加项目信息
     */
    @GetMapping("/getAcademicPerformance/exportParticipateProject")
