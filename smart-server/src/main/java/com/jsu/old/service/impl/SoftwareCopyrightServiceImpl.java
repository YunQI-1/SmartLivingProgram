...
import com.jsu.old.mapper.SoftwareCopyrightMapper;
import com.jsu.old.service.SoftwareCopyrightService;
...
public class SoftwareCopyrightServiceImpl extends ServiceImpl<SoftwareCopyrightMapper, SoftwareCopyright> implements SoftwareCopyrightService {
    ...
    @Override
    public void exportSoftwareCopyright(HttpServletResponse response, ExportConfigDTO<SoftwareCopyrightDTO> exportConfigDTO) {
         QueryWrapper<SoftwareCopyright> wrapper=new QueryWrapper<>();
         QueryUtils.buildFuzzyQuery(exportConfigDTO.getQueryParams(),wrapper);
         List<SoftwareCopyright> list=softwareCopyrightMapper.selectList(wrapper);
         ExcelUtils.exportWithDynamicColumns(
          response,
                 "软著表.xlsx",
                 list,
                 SoftwareCopyright.class,
    ...
    @Override
    public List<SoftwareCopyrightVO> getSoftwareCopyrightDetail(QueryDTO queryDTO) {
        return softwareCopyrightMapper.getSoftwareCopyrightDetail(queryDTO);
    }
}